...
import Component from '../../../../../omelette/src/components/theme/Component/Component';
...
const Body = ({ data, isEditMode }) => {
  ...
  const allowedPlayersList = config.blocks.blocksConfig?.cmsharevideo?.whiteList || [];
  const allowedPlayers = allowedPlayersList.map(playerName => players[playerName]);

  console.log({allowedPlayersList, players, allowedPlayers})
  ...
  return (
    <>
      {data.url && (
        ...
        >
          {allowedPlayers.map(Component => (
            <Component data={data} embedSettings={embedSettings} />
          ))}
